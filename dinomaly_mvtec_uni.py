...
def train(item_list, run_args=None):
    setup_seed(1)
    ...
    # Append average summary to log file (same format as mvtec_log.txt)
    if run_args is not None and final_means is not None:
        log_path = os.path.join(run_args.save_dir, run_args.save_name, 'log.txt')
        try:
            with open(log_path, 'a', encoding='utf-8') as f:
                f.write(format_mean_summary(*final_means))
        except Exception as e:
            print('Failed to append mean summary to {}: {}'.format(log_path, e))
